...
A full description of methods and results can be found at [haroonmoh/catan-ai-cpsc474-574](https://github.com/haroonmoh/catan-ai-cpsc474-574). 

## Packages to install 

Q-Learning: matplotlib, numpy, pygame
DQN: tensorflow, tqdm, gym
...
```code
make test-all
make test-q-learning
make test-rl-agent
make test-rl-agent-long
make test-head-to-head
```
- test-all runs both q-learning and rl-agent
- rl-agent-long takes an hour vs rl-agent takes 1 minute
- make test-head-to-head pits the q-learning model vs rl-agent
...
### Code Structure
- notebooks/train_dqn_catan_colab.ipynb
- code/catanDQNEnv.py

The code/catanDQNEnv.py sets up the gym environment for me to train in. This file is AI generated for the most part. This sets up the environment for the actual DQN to operate in.

The second, more important file is notebooks/train_dqn_catan_colab.ipynb. This is the file that I made to train my DQN in Google Colab. It should work out of the box if you upload it to Colab and run it. The code follows the pseudocode from our final lecture in class with the slight variance of having a action mask. I talk about why I add the action mask in the discussion section. 

**But to train simply upload the file to Colab and run!**

Note: What is nice about this is that I can easily switch the env and use my train_dqn_catan_colab to train on a different environment because my code abstracts everything from the environment away.
...
I also played around with the size of the neural network to see whether it had any impact. I did not notice a huge difference between the different sizes.

The third major change that I had to make was adding an action mask. Initially, I did not have one and it would learn to take 10-20 invalid actions and then a valid action. It lost pretty much every single game. So I updated it to have a mask so that I could avoid the invalid actions!
